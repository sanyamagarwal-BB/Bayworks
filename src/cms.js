...
export const DEFAULTS = {
  ...
  metric_4_val:   '21 days', metric_4_label: 'Fastest deal closure on record',
  ...
  clients_list:   JSON.stringify(['TCS', 'Infosys', 'HCL', 'Wipro', 'Accenture', 'Goldman Sachs', 'Morgan Stanley', 'Deloitte', 'EY', 'Capgemini']),
  ...
  if (clientsContainer && d.clients_list) {
    try {
      ...
      const html = (clients.concat(clients)) // duplicate for seamless loop
        .map(c => `<div class="client-logo">${c.replace(/\n/g, '<br>')}</div>`)
        .join('');
