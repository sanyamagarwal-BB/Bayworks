...
views.signup.addEventListener('submit', async (e) => {
  ...
  const data = { name: $('#su-name').value, company: $('#su-company').value, email: $('#su-email').value, phone: $('#su-phone').value, password: $('#su-pass').value };
  let ok = true;
...
$('#si-forgot').addEventListener('click', async () => {
  ...
  try {
    await import('./partner-api.js').then(m => m.forgotPassword(email));
